...
class SettingController extends Controller
{
    ...
    public function general(Request $request)
    {
        ...
        if (!$request->isMethod('post'))
        {
            ...
            $escrow        = Setting::where('type', 'escrow')->get()->toArray();
            $data['escrow'] = $result = $this->helper->key_value('name', 'value', $general);
    ...
    public function escrow(Request $request)
    {
        ...
        $data['menu'] = 'settings';
        $data['settings_menu'] = 'escrow';
        
        if ($request->isMethod('post'))
        {   

            // Admin Single ip address access
            if (! empty($request->get('escrow_wallet_id'))) {
                if(!$escrow_wallet_id = Setting::where(['name' => 'escrow_wallet_id', 'type' => 'escrow'])->update(['value' => $request->escrow_wallet_id]) ){
                    Setting::create(['name' => 'escrow_wallet_id', 'type' => 'escrow', 'value' => $request->escrow_wallet_id]);
                }

            } else {
                $this->helper->one_time_message('error', 'You have provide escrow wallet id');
                return redirect(Config::get('adminPrefix').'/settings/escrow');
            }


            $this->helper->one_time_message('success', 'Escrow settings updated successfully.');
            return redirect(Config::get('adminPrefix').'/settings/escrow');
        }

        $settings        = Setting::where('type', 'escrow')->get()->toArray();
        $data['result'] = $this->helper->key_value('name', 'value', $settings);


        return view('admin.settings.escrow', $data);
    ...
    public function payment_methods(Request $request)
    {
        if (!$request->isMethod('post'))
        {
            ...
            $data['flutterwave']      = $paypal      = Setting::where('type', 'Flutterwave')->pluck('value', 'name', 'id')->toArray();
