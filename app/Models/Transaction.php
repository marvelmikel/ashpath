...
/*Start of relationships*/
    ...
    public function cryptoapi_log()
    {
        if (config('crypto_exchange.is_active')) {
            return $this->hasOne(CryptoapiLog::class, 'object_id')->whereIn('object_type', ["crypto_sent", "crypto_received", "crypto_exchange_from", "crypto_exchange_to", "fiat_exchange_from", "fiat_exchange_to"]);
        }
    ...
    public function getTransactions($from, $to, $type, $wallet, $status)
    {
        ...
        if (!empty($type) && $type != 'all')
        {
            ...
            else
            {
                ...
                elseif ($type == 'crypto-to-fiat-exchange')
                {
                    $whereInCondition = [Crypto_Exchange_From, Crypto_Exchange_To];
                }
                elseif ($type == 'fiat-to-crypto-exchange')
                {
                    $whereInCondition = [Fiat_Exchange_From, Fiat_Exchange_To];
                }
            }
        }
